...
#获取数据的函数，有两种，一种是简单获取，另一种是从pascal_voc获取
#无论那种获取方法，返回的都是
#all_data：[{filename:{filepath:*,width:*,height:*,bboxes:['class':*, 'x1': *, 'x2': *, 'y1':*, 'y2':*]}}]
...
C.num_rois = int(options.num_rois)  #估计是在RoI pooling层使每个RoI生成固定尺寸的feature map时用的


#nn是网络结构
#分别有几个函数：
# get_weight_path  ：返回存储的模型文件名称
# get_img_output_length #返回整除16后的长宽
# nn_base ：共享基础网络
